        ...
        Campsite.findById(req.params.campsiteId)
            ...
            .then(campsite => {
                if (campsite && campsite.comments.id(req.params.commentId)) {
                    ...
                    const err = new Error(`Campsite campsites/${req.params.campsiteId} not found`)
                    err.status = 404;
                    next(err)
                }
                else {
                    const err = new Error(`Can not find campsites/${req.params.campsiteId}/comments/${req.params.commentId}`)
                }
            })
            .catch(err => next(err))
    })
